...
from butterfly.db.api.constants import OPENED_AT_KEY, COMPLETED_AT_KEY, TIME_FORMAT
...
def activity_lesson_get_all_post(user_id):
    ...
    def get():
        ...
        for activity_lesson in activity_lesson_list:
            activity_lesson["url"] = "{}/{}".format(request.url, activity_lesson.get("lesson_id"))
            activity_lesson[OPENED_AT_KEY] = activity_lesson[OPENED_AT_KEY].strftime(TIME_FORMAT)
            if activity_lesson[COMPLETED_AT_KEY]:
                activity_lesson[COMPLETED_AT_KEY] = activity_lesson[COMPLETED_AT_KEY].strftime(TIME_FORMAT)
